...
class UserContextDB {
  ...
  /**
   * Atualiza o relacionamento com Kaiser
   */
  updateRelationship(userId, campo, valor) {
    const context = this.getUserContext(userId);
    
    if (context.relacionamento_kaiser.hasOwnProperty(campo)) {
      context.relacionamento_kaiser[campo] = valor;
      context.ultima_atualizacao = getBrazilDateTime();
      this.saveDatabase();
    }
  }
  ...
  addSpecialMemory(userId, memoria) {
    ...
    context.relacionamento_kaiser.memorias_especiais.push(novaMemoria);
    
    // Manter apenas as 30 memórias mais especiais
    if (context.relacionamento_kaiser.memorias_especiais.length > 30) {
      context.relacionamento_kaiser.memorias_especiais = 
        context.relacionamento_kaiser.memorias_especiais.slice(-30);
    }
  ...
  updateMemory(userId, tipo, valorAntigo, valorNovo) {
    ...
    switch (tipoNormalizado) {
      ...
      case 'memória':
        const indexMemoria = context.relacionamento_kaiser.memorias_especiais.findIndex(
          m => m.texto === valorAntigo
        );
        if (indexMemoria !== -1) {
          context.relacionamento_kaiser.memorias_especiais[indexMemoria].texto = valorNovo;
          context.relacionamento_kaiser.memorias_especiais[indexMemoria].data = getBrazilDateTime();
          atualizado = true;
        }
        break;
  ...
  deleteMemory(userId, tipo, valor) {
    ...
    switch (tipoNormalizado) {
      ...
      case 'memória':
        ...
        if (indexMemoria !== -1) {
          context.relacionamento_kaiser.memorias_especiais.splice(indexMemoria, 1);
          removido = true;
        }
        break;
  ...
  getUserContextSummary(userId) {
    ...
    const summary = {
      ...
      nivel_intimidade: context.relacionamento_kaiser.nivel_intimidade,
      ...
      memorias_especiais: context.relacionamento_kaiser.memorias_especiais.slice(-5).map(m => m.texto).join('\n- ') || 'Nenhuma'
    };
